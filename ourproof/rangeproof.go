...
import (
	...
	"github.com/dcrpalg/gmsm/sm3"
	"github.com/dcrpalg/ourproof/wire"
)

// The following was copy-pasted from currency's original implementation by Willy (github.com/wrv)
// TODO: replace rangeproofs of currency with bulletproofs, eventually
...
func proofGenA(zkpcp ZKPCurveParams,
	...
	if !bit { // If bit is 0, just make a random R = k*H
		...
		hash := sm3.Sm3Sum(append(temp.X.Bytes(), temp.Y.Bytes()...))
...
func proofGenB(zkpcp ZKPCurveParams,
	...
	if !bit {
		...
		hash := sm3.Sm3Sum(append(totX.Bytes(), totY.Bytes()...))
...
func verifyGen(zkpcp ZKPCurveParams,
	...
	hash := sm3.Sm3Sum(append(tot.X.Bytes(), tot.Y.Bytes()...))
