...
//                                          s1G + s2H ?= T1 + cCM
//                                          s2PK ?= T2 + cCMTok
//通过证明两个值都等于一个中间值来证明consistency
//Proof就是一串能够通过某些运算证明它们满足某些确定关系的数据
type ConsistencyProof struct {
	T1        ECPoint
	T2        ECPoint
	Challenge *big.Int //c
	S1        *big.Int // s1 - but capitalized to allow access from outside of ourproof
	S2        *big.Int // s2 - but capitalized to allow access from outside of ourproof
}
