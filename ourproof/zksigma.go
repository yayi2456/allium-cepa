...
ourproof is a library for generating non-interactive zero-knowledge proofs, also known as NIZKs. The proofs in ourproof are based on Generalized Schnorr Proofs; they can be publicly verified and do not require any trusted setup.
...
import (
	...
	"github.com/dcrpalg/ourproof/btcec"
)
...
// It is equal to currency's curve - but for abstraction the actual curve parameters are
// passed into the proof functions. We just test with the same params that currency uses.
...
func init() {
	s256 := sha256.New()
	hashedString := s256.Sum([]byte("This is the new random point in ourproof"))
