...
func (p *P256) Neg(a *P256) *P256 {
    ...
    mone := new(big.Int).Sub(CURVE.Params().N, one)
    p.ScalarMult(p, mone)
    return p
}
...
func (p *P256) Add(a, b *P256) *P256 {
    ...
    resx, resy := CURVE.Sm2Add(a.X, a.Y, b.X, b.Y)
    p.X = resx
    p.Y = resy
    return p
}
...
func (p *P256) Double(a *P256) *P256 {
    ...
    resx, resy := CURVE.Sm2Double(a.X, a.Y)
    p.X = resx
    p.Y = resy
    return p
}
...
func (p *P256) ScalarMult(a *P256, n *big.Int) *P256 {
    ...
    n = bn.Mod(n, CURVE.Params().N)
    bns := n.Bytes()
    resx, resy := CURVE.Sm2ScalarMult(a.X, a.Y, bns)
    p.X = resx
    p.Y = resy
    return p
}
...
func (p *P256) ScalarBaseMult(n *big.Int) *P256 {
    ...
    n = bn.Mod(n, CURVE.Params().N)
    bns := n.Bytes()
    resx, resy := CURVE.Sm2ScalarBaseMult(bns)
    p.X = resx
    p.Y = resy
    return p
}
...
func MapToGroup(m string) (*P256, error) {
    ...
    for i < 256 {
        ...
        x = bn.Mod(x, CURVE.Params().P)
        fx, _ := F(x)
        fx = bn.Mod(fx, CURVE.Params().P)
        y := fx.ModSqrt(fx, CURVE.Params().P)
        if y != nil {
            p := &P256{X: x, Y: y}
...
func (p *P256) IsOnCurve() bool {
    ...
    //
    //return x3.Cmp(y2) == 0
   return CURVE.Sm2IsOnCurve(p.X,p.Y)
}
